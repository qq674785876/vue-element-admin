  ...
  <div v-loading="loading">
    ...
    >
      <div v-if="!isGet" class="packageList">
        ...
        <el-row class="package-info" v-if="adType == 0">
          ...
          <div>
            ...
            <div>
              ...
              <!-- <el-radio-group
                v-model="adModelId"
                size="mini"
                @change="selectRow('adModel', adModel, adModelId)"
              >
                <el-radio-button
                  :label="list.propertyId"
                  border
          ...
          <div>
            ...
            <el-radio-group
              v-model="monthId"
              size="mini"
              @change="selectRow('month', packageObj.package, monthId)"
          ...
          <div>
            ...
            <el-radio-group
              v-model="frequencyId"
              size="mini"
              @change="selectRow('frequency', packageObj.frequency, frequencyId)"
              :disabled="adModelId.length === 0"
            >
              <el-radio-button
                :label="list.propertyId"
                border
                v-for="(list,index) in packageObj.frequency"
                :key="index"
              >{{list.name}}</el-radio-button>
            </el-radio-group>
        ...
        <el-row v-else class="package-info">
          <div>
            <span class="title">
              屏蔽本应用下载中的广告
              <i>（说明：屏蔽广告后可让本应用使用纯净模式展现给下载用户）</i>
            </span>
            <el-radio-group
              v-model="monthId"
              size="mini"
      ...
      <div v-if="isGet && !isPuySuccess" align="center">
        <div>
          请用
          <span style="color: blue;font-size: 16px;">微信扫码</span>
          支付￥{{ buyPrice }}
        </div>
        <div class="QRCode_box" style="width: 200px;height: 200px;margin: 20px auto;">
          <img :src="qrCode" style="width: 100%;">
        </div>温馨提示：
        <p class="tips" style="color: red;">支付成功后自动刷新</p>
      </div>
      <div v-if="isPuySuccess" align="center">
        <i class="el-icon-success" style="font-size: 50px;color: green;"/>
        <p>套餐购买成功</p>
      </div>
...
export default {
  ...
  methods: {
    modeChange(){
      ...
      for(var i = 0 ; i < _this.adModelId.length; i++){
        for(var x = 0 ; x < _this.adModel.length; x++){
          if(_this.adModelId[i] === _this.adModel[x].propertyId){
            price += Number(_this.adModel[x].price);
          }
        }
      }

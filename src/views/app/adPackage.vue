  ...
  <div v-loading="loading">
    <el-dialog
      ...
      <div v-if="!isGet" class="packageList">
        <el-row>
          <el-radio-group v-model="adType" @change="packageInfo" size="mini">
        ...
        <el-row class="package-info" v-if="adType == 0">
          ...
          <div>
            ...
            <el-radio-group v-model="propertyId" size="mini" @change="selectRow(propertyId)">
              <el-radio-button 
                v-if="frequency === list.frequencyText" 
                :label="list.propertyId"
                border
                v-for="(list,index) in packageObj.package" 
                :key="index">{{list.price + ' / ' + list.month + '月'}}</el-radio-button>
            </el-radio-group>
          </div>
          <div>
            <span class="title">展示频率</span>
            <el-radio-group v-model="frequency" size="mini" @change="propertyId = ''">
              <el-radio-button :label="list" border v-for="(list,index) in packageObj.frequency" :key="index">{{list}}</el-radio-button>
            </el-radio-group>
          </div>
        </el-row>
...
export default {
  ...
  methods: {
    ...
    selectRow(id) {
      const _this = this;
      let packageList = _this.packageObj.package;
      for(let i = 0;i < packageList.length;i++){
        if(id === packageList[i].propertyId){
          _this.buyPackage = packageList[i]
          _this.buyPrice = packageList[i].price
          break;
        }
      }
    },
